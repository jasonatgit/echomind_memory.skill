...
| **统一 RL 评分（v1.2.13+）** | 五个权重维度驱动所有被排序来源：freshness（艾宾浩斯）为唯一时间衰减杠杆、`trust_score` 参与 experience 排序、各来源信号按实际权重归一——加权和内无固定常数 |
| **来源追溯（v1.2.14+）** | 每条记忆自描述来源——传输方式（mcp/http/hermes/cli）+ 客户端（claude-code/opencode/...，MCP 握手自动推断）+ project + tags + 捕获时间；支持按 tags（OR/AND）、来源、日期区间及其任意组合经 `echomind_query` 检索 |
...
| 版本 | 核心要点 |
|:--------|:-----------|
| v1.2.18 | *内部重构（Top-2 大文件拆分）：从两个最大模块中抽出纯评分/新颖度助手、生命周期/进化/实体子 Agent、存储 schema/keys/rows（MainMemoryAgent 2959→2578，SqliteStore 2370→1972）——行为不变，全部调用点经委派/重导保留，新增 39 项评分数值单测（74→113）。* |
| v1.2.17 | *对 v1.2.16 的复核轮 — 全部修复逐个对照真实引擎重验：修复 TTL 绑定数缺陷、优化延迟回收、配置回退一致性复核与反思 batch 钳制复核。* |
| v1.2.16 | *全数据链路审计：退出契约统一、LIMIT 下稀疏标签检索、reflections profile 级删除、删除级联一致、时间戳统一、状态机跨用户隔离、反思 knowledge 持久化与租户化、反思幂等、RL 锁加固、MCP/HTTP/CORS 加固、缓存限界。* |
| v1.2.15 | *深度审计修复：自动反思 / 生命周期状态机 / 知识进化检测死路径复活、知识主键租户隔离、MCP 客户端按会话隔离 + `/mcp` 鉴权、GSPO 改可选、检索路径 LLM 门控（最坏 181s → 5s）。* |
| v1.2.14 | *来源追溯：origin 追踪（传输方式 + 客户端，MCP clientInfo 推断）、tags 过滤（调用方 tags + 自动主题 tags，OR/AND）、来源信封、来源行可见、结构化 `echomind_query`。* |
| v1.2.13 | *深度审查加固：租户隔离、RL 读并发、反思配额原子化、评分统一（freshness 单杠杆、trust 驱动 experience）、配置校验与安全加固。* |
| v1.2.12 | *核心词新颖比例、RL 显著性验证、代码块安全分块。* |
| v1.2.11 | *项目作用域、Hermes 分身隔离、content-hash 知识去重。* |
...
4. **全链路分身隔离（v1.2.13+）**：知识去重、反馈 RL 权重、删除（单条与用户级）均按 profile 隔离
